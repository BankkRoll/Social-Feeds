...
const Settings: React.FC<SettingsProps> = ({ userData }) => {
  ...
  const [tweets, setTweets] = useState<Tweets>(userData.tweets || { 
  ...
  const handleTweetChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement>, tweetKey: keyof Tweets, field: 'id' | 'active') => {
  ...
  const handleSocialChange = (e: ChangeEvent<HTMLInputElement>, platform: keyof Socials) => {
    const { name, value } = e.target;
  ...
  const handleSocialActiveChange = (e: ChangeEvent<HTMLSelectElement>, platform: keyof Socials) => {
  ...
  // TODO: handle image uplaod toa  url then to firbase as a url
  const handleImageChange = (e: ChangeEvent<HTMLInputElement>, setImageFile: React.Dispatch<React.SetStateAction<File | null>>) => {
  ...
  const handleFormSubmit = async (e: FormEvent) => {
    ...
    if (address && db) {
      ...
      const q = query(usersRef, where("profile.userName", "==", profile.userName));
      ...
      // If the username is not taken, proceed to save the profile
      ...
      try {
        ...
        const copyToClipboard = () => {
          ...
          // Trigger a new toast here to notify the user that the URL was copied
          toast({
            variant: "success",
            title: "Copied",
  ...
  const handleDeleteAccount = async () => {
    ...
    if (deleteStep === 2) {
      ...
      setDeleteStep(0);  // Reset the delete step after completion or error
    }
  };
  ...
  <div className="relative cursor-pointer inline-block">
    <img 
      src={avatarFile ? URL.createObjectURL(avatarFile) : profile.avatarImg} 
      alt="Avatar" 
  ...
  <div className="relative cursor-pointer inline-block">
    <img 
      src={bannerFile ? URL.createObjectURL(bannerFile) : profile.bannerImg} 
      alt="Banner" 
...
const { active, title, description, thumbnailurl, siteurl } = details!;
return (
    <AccordionItem value={platform} key={platform}>
      <AccordionTrigger>{platform}</AccordionTrigger>
      <AccordionContent>
            <div className="flex flex-row mb-2">
                  <label className="block text-lg font-semibold m-2">Active</label>
                  <select 
...
<Accordion type="single" collapsible>
  ...
  {Object.keys(tweets).map((tweetKey) => (
    <div key={tweetKey} className="flex items-center justify-between mb-4">
      <div className="flex-1 mr-2">
        <label htmlFor={tweetKey} className="block text-lg font-semibold mb-2">{tweetKey}</label>
        <Input
          id={tweetKey}
          name={tweetKey}
  ...
  <div className="flex justify-center space-x-4">
    ...
    >
      {deleteStep === 0 ? 'Delete Account' : deleteStep === 1 ? 'Are you sure?' : 'Confirm Deletion'}  {/* Update the button text based on deleteStep */}
    </Button>
  </div>
